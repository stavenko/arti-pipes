...
/// Builder for GptOss executor
pub struct GptOssBuilder {
...
impl GptOssBuilder {
    ...
    fn into_config(self) -> GptOssConfig {
        GptOssConfig {
    ...
    pub fn build_with_transport<T: HttpTransport>(self, transport: T) -> GptOss<T> {
        GptOss {
            transport,
            config: self.into_config(),
        }
...
impl Default for GptOssBuilder {
    fn default() -> Self {
        Self::new()
    }
}

/// GPT-OSS-specific chat completion request with `think` field for Ollama
#[derive(Debug, Serialize)]
struct GptOssChatCompletionRequest {
    model: String,
    messages: Vec<ChatMessage>,
    #[serde(skip_serializing_if = "Option::is_none")]
    response_format: Option<ResponseFormatRequest>,
    stream: bool,
    /// Ollama-specific: thinking mode for GPT-OSS ("low", "medium", "high")
    #[serde(skip_serializing_if = "Option::is_none")]
    think: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    max_tokens: Option<u32>,
...
/// GPT-OSS LLM provider, generic over the HTTP transport.
///
/// Defaults to the platform's [`DefaultTransport`]; tests can inject a mock by
/// building via [`GptOssBuilder::build_with_transport`].
#[derive(Clone)]
pub struct GptOss<T = DefaultTransport> {
    transport: T,
    config: GptOssConfig,
}

impl GptOss<DefaultTransport> {
    /// Create a new builder for GptOss
    pub fn builder() -> GptOssBuilder {
        GptOssBuilder::new()
    }
}
...
impl<T: HttpTransport> crate::executor::PromptExecutor for GptOss<T> {
...
impl<T: HttpTransport> GptOss<T> {
    async fn execute_internal(
        &self,
        prompt: String,
        response_format: Option<ResponseFormatRequest>,
    ) -> Result<ExecutionResult<String>> {
        let request_body = GptOssChatCompletionRequest {
