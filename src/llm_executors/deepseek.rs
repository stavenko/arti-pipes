...
impl DeepSeekBuilder {
    ...
    pub fn build_with_transport<T: HttpTransport>(self, transport: T) -> DeepSeek<T> {
        DeepSeek {
            transport,
            config: self.into_config(),
        }
...
/// building via [`DeepSeekBuilder::build_with_transport`].
#[derive(Clone)]
pub struct DeepSeek<T = DefaultTransport> {
    transport: T,
    config: DeepSeekConfig,
}

impl DeepSeek<DefaultTransport> {
...
impl<T: HttpTransport> crate::executor::PromptExecutor for DeepSeek<T> {
...
impl<T: HttpTransport> DeepSeek<T> {
